...
class _AddExpenseScreenState extends State<AddExpenseScreen> {
  ...
  final List<String> categories = ['food', 'shopping', 'transport', 'bills', 'entertainment', 'other'];
  ...
  Future<void> _submitAll() async {
    ...
    try {
      ...
      // ✅ تسجيل Transaction واحد بالإجمالي فقط
      await FirebaseFirestore.instance
          .collection('users')
          .doc(user.uid)
          .collection('transactions')
          .add({
        'title': 'Multiple Expenses (${titleControllers.length} items)',
        'amount': total,
        'type': 'expense',
        'category': 'multiple',
